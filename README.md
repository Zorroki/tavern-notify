...
- 可在“重新生成”所在的生成菜单里加入“刷新页面”选项，方便需要时手动重载当前页面
- 提供卡顿诊断面板，手动采集长任务、长帧、滚动位置、消息数量和后台任务数量
- 返回酒馆页面后自动同步后台结果
...
3. 如需快速手动重载页面，可以保留 `在生成菜单显示“刷新页面”`
4. 如需排查滚动卡顿，可以在 `卡顿诊断` 中点击 `开始采样`，复现卡顿后点击 `停止采样` 并 `复制报告`
5. 在 `通知方式` 里选择 `Bark` 或 `网页通知`
6. 如果选的是 `Bark`
   - 填 `Bark 服务地址`
   - 填 `Bark Device Key`
   - `Bark 图标地址` 留空时会优先尝试当前酒馆页面里的 PNG / ICO 图标
   - 如果手动填写图标，建议使用可直接访问的 `HTTP(S)` 图片地址；`SVG`、`localhost` 或容器内地址在 Bark 里经常不会显示
   - Bark 自定义图标仅 iOS 15 或以上支持，并且会按图标 URL 在设备端缓存；图标改了但不生效时，建议换一个 URL 或追加 `?v=2`
...
- 卡顿诊断只在手动点击 `开始采样` 后运行，不记录聊天正文；复制报告用于判断卡顿更像是主线程长任务、掉帧、长聊天 DOM 或后台任务同步导致。
...
如果开启了 `在生成菜单显示“刷新页面”`，还可以在“重新生成”所在的生成菜单里手动触发一次页面刷新。
